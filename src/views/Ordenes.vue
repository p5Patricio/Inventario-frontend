        ...
        <div class="modal">
          ...
          <form class="modal-body" @submit.prevent="submitForm">
            ...
            <footer class="modal-footer">
              ...
              <button class="add-btn" type="submit" :disabled="isSubmitting">{{ isEdit ? "Update Order" : "Add Product" }}</button>
            </footer>
          </form>
        </div>
      </div>
...
export default {
  components: { Userbar, BarraLateral },
  data() {
    return {
      showModal: false,
      isEdit: false,
      isSubmitting: false, // Flag para deshabilitar el botón mientras se está enviando
      editingOrderId: null,
      orders: [],
      form: {
        productName: "",
      ...
      fields: [
        ...
        { label: "Quantity", id: "quantity", type: "number", model: "quantity", placeholder: "Enter quantity" },
        ...
        { label: "Date of Delivery", id: "delivery-date", type: "date", model: "deliveryDate" },
      ],
    };
  },
  computed: {
    ...
    totalReceived() {
      return this.orders.reduce((acc, order) => acc + parseFloat(order.value.replace("₹", "").replace(",", "")), 0);
    },
    totalReturned() {
      return this.orders.filter(order => order.status === "Returned").length;
    },
    totalOnTheWay() {
      return this.orders.filter(order => order.status === "On the way").length;
    },
  },
  methods: {
    async fetchOrders() {
      try {
        const response = await axios.get("http://localhost:5000/ordenes/list");
    ...
    async submitForm() {
      if (!this.form.productName || !this.form.orderValue || !this.form.quantity) {
        alert("Please fill all required fields.");
        return;
      }

      this.isSubmitting = true; // Habilita la bandera de carga
      const payload = { ...this.form };

      try {
        console.log("Submitting form:", payload);  // Verificación del payload

        if (this.isEdit) {
          console.log("Updating order with ID:", this.editingOrderId);
          await axios.put(`http://localhost:5000/ordenes/update/${this.editingOrderId}`, payload);
          const index = this.orders.findIndex(order => order.id === this.editingOrderId);
          if (index !== -1) {
            this.orders[index] = { ...payload, id: this.editingOrderId };
          }
        } else {
          const response = await axios.post("http://localhost:5000/ordenes/add", payload);
        ...
        this.isSubmitting = false; // Deshabilita la bandera de carga
      }
    },
    async deleteOrder(orderId) {
      try {
        await axios.delete(`http://localhost:5000/ordenes/delete/${orderId}`);
